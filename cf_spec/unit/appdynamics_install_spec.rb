  ...
  context 'vcap services' do
    ...
    context 'contains appdynamics' do
      ...
      context 'APPDYNAMICS_AGENT_ACCOUNT_ACCESS_KEY set' do
        ...
        it "does not modify APPDYNAMICS_AGENT_ACCOUNT_ACCESS_KEY" do
          subject
          expect(@status).to be_success
          profile_d_contents = File.read(profile_d_appdynamics)
          expect(profile_d_contents).to_not include('APPDYNAMICS_AGENT_ACCOUNT_ACCESS_KEY')
        end
      end
      ...
      context 'APPDYNAMICS_AGENT_APPLICATION_NAME set' do
        ...
        it "does not modify APPDYNAMICS_AGENT_APPLICATION_NAME" do
          subject
          expect(@status).to be_success
          profile_d_contents = File.read(profile_d_appdynamics)
          expect(profile_d_contents).to_not include('APPDYNAMICS_AGENT_APPLICATION_NAME')
        end
